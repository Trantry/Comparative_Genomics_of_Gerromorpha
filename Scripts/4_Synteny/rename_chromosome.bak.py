...
def parse_whitespace_table(path):
    """
    Parse a table where columns are separated by arbitrary whitespace (tabs or spaces).
    ...
    with p.open("r", encoding="utf-8", errors="ignore") as fh:
        for ln in fh:
            # remove trailing and leading spaces/newlines but preserve interior tokens
            line = ln.rstrip("\n").strip()
            if line == "":
                continue
            # split on 1+ whitespace (tabs or spaces)
